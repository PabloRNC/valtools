          ...
          case "ready_for_auth": {
            ...
            socket.send(
              JSON.stringify({
                ...
                payload: { url: connections.get(identity)!.payload.channel_id === 'mock' ? `https://auth.riotgames.com/authorize?client_id=${process.env.RSO_CLIENT_ID}&redirect_uri=${process.env.RSO_REDIRECT_MOCK_URI}&response_type=code&scope=openid+offline_access&state=${state}` :`https://auth.riotgames.com/authorize?client_id=${process.env.RSO_CLIENT_ID}&redirect_uri=${process.env.RSO_REDIRECT_URI}&response_type=code&scope=openid+offline_access&state=${state}` },
              })
            );
